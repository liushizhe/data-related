function calCrc () {
	var finput = document.getElementById("data_crc_bin");
	var editor = document.getElementById("editor_file");
...
function crcProc(data){
	...
	// hexCharCode.push("0x");
	// for(var i = 1; i >=0; i--) {
 //     hexCharCode.push(((crc_tmp >> 8*i).toString(16));
 //    }
...
function doSave(value, type, name) {  
    ...
    if (typeof window.Blob == "function") {  
        blob = new Blob([value], {type: type}); //创建Blob对象，value可以是字符串或Uint8Array等
    } else {  
    ...
    var bloburl = URL.createObjectURL(blob); //下载路径指向该文件对象
